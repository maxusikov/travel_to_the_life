<?php
class ControllerExtensionThemeThemeLifetravel extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('extension/theme/theme_lifetravel');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('theme_lifetravel', $this->request->post, $this->request->get['store_id']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/theme/theme_lifetravel', 'token=' . $this->session->data['token'] . '&store_id=' . $this->request->get['store_id'], true)
		);

		$data['action'] = $this->url->link('extension/theme/theme_lifetravel', 'token=' . $this->session->data['token'] . '&store_id=' . $this->request->get['store_id'], true);

		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=theme', true);

		if (isset($this->request->get['store_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$setting_info = $this->model_setting_setting->getSetting('theme_lifetravel', $this->request->get['store_id']);
		}
		...
		$this->response->setOutput($this->load->view('extension/theme/theme_lifetravel', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/theme/theme_lifetravel')) {
